...
function StatusPicker({ value, itemType, onChange }: { value: string; itemType: string; onChange: (s: string) => void }) {
        ...
        <button
          ...
          className={cn(
            ...
            value === s
              ...
              : 'border-black/10 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-600',
          )}>
  ...
  return (
    <button onClick={onClick} className="text-left group">
      ...
      <div className="mt-1.5 px-0.5">
        <p className="text-xs font-semibold leading-tight line-clamp-2 text-gray-900 dark:text-gray-100">{item.title}</p>
...
function AddSheet({ defaultTab, onClose, onAdded }: {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        else if (!res.ok && data?.hint)    { setSearchError(`${data.error} — ${data.hint}`) }
        else if (!res.ok)                  { setSearchError(data?.error ?? `Search failed (${res.status})`) }
        else                               { setResults([]) }
      } catch { setSearchError('Search unavailable — check your connection') }
  ...
  async function selectResult(r: SearchResult) {
    if (r.type === 'book') { setSelected(r); return }
    setLoadingDetails(true)
    try {
      const res = await fetch(`/api/life/tmdb/details?id=${r.id}&type=${r.type}`)
      setSelected(res.ok ? { ...r, ...await res.json() } : r)
  ...
  async function save() {
    ...
    try {
      const res = await fetch('/api/life/watchlist', {
        ...
        body: JSON.stringify({
          type: selected.type, status, title: selected.title,
          externalId: String(selected.id), posterUrl: selected.posterUrl,
          year: selected.year, genres: selected.genres, overview: selected.overview,
          author: selected.author ?? null, tmdbRating: selected.tmdbRating,
          streamingOn: selected.streamingOn ?? [], myRating, notes: notes.trim() || null,
        }),
      })
      ...
      <div className="relative bg-white/85 dark:bg-gray-900/70 rounded-t-2xl max-h-[88vh] flex flex-col shadow-2xl">
        ...
        <div className="flex-none flex items-center justify-between px-5 py-3">
          <h2 className="font-bold text-lg text-gray-900 dark:text-white">
            {defaultTab === 'book' ? 'Add Book' : 'Add to Watchlist'}
          </h2>
        ...
        {/* Tab pills — only shown on watchlist (media) where switching to book is still useful */}
        {defaultTab === 'media' && (
        ...
        {selected ? (
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-6">
              {loadingDetails || searching ? (
                <div className="flex items-center justify-center py-8 gap-2 text-gray-400">
                  <Loader2 size={20} className="animate-spin" />
                  <span className="text-sm">{loadingDetails ? 'Fetching details…' : 'Searching…'}</span>
                </div>
              ) : searchError ? (
...
function DetailSheet({ item, onClose, onUpdated, onDeleted }: {
  item: WatchlistItem
  onClose: () => void
            ...
            {confirmDelete ? (
              <div className="flex gap-2">
                <button onClick={() => setConfirmDelete(false)}
                  className="flex-1 py-2.5 rounded-xl border border-black/10 dark:border-white/10 text-sm text-gray-600 dark:text-gray-400 font-medium">
                  Cancel
                </button>
                <button onClick={remove} disabled={deleting}
                  className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition-colors disabled:opacity-60 flex items-center justify-center gap-1.5">
...
interface WatchlistContentProps {
  forceType: 'media' | 'book'
}

export function WatchlistContent({ forceType }: WatchlistContentProps) {
  ...
  const filtered = useMemo(() => {
    let list = items.filter(i => isBook ? i.type === 'book' : i.type === 'movie' || i.type === 'tv')
    ...
    return list
  }, [items, isBook, statusFilter, search])

  const tabItems = useMemo(() =>
    items.filter(i => isBook ? i.type === 'book' : i.type === 'movie' || i.type === 'tv'),
    [items, isBook],
  )

  const counts = useMemo(() => {
    const c: Record<string, number> = {}
    for (const s of Object.keys(STATUS_CFG)) c[s] = tabItems.filter(i => i.status === s).length
  ...
  const STATUS_FILTERS = [
    { key: null,        label: 'All' },
    { key: 'want_to',   label: isBook ? 'Want to Read' : 'Want to Watch' },
    { key: 'currently', label: isBook ? 'Reading' : 'Watching' },
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {isBook ? 'Books' : 'Movies & TV'}
          </h1>
          <p className="text-sm text-gray-400 mt-0.5">
            {isBook ? 'Your reading list' : 'Movies and shows to watch'}
          </p>
        </div>
      ...
      {tabItems.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-4 px-4 scrollbar-hide">
          {STATUS_FILTERS.map(f => {
            ...
            return (
              <button key={String(f.key)} onClick={() => setStatusFilter(f.key)}
                className={cn(
      ...
      {loading ? (
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
          {[1,2,3,4,5,6].map(i => <div key={i} className="aspect-[2/3] bg-gray-200 dark:bg-gray-800 rounded-xl animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-16 text-center">
          <p className="text-4xl mb-3">{isBook ? '📚' : '🎬'}</p>
          {tabItems.length === 0 ? (
            <>
              <p className="text-gray-500 font-medium">Nothing here yet</p>
              <p className="text-sm text-gray-400 mt-1">Hit <strong>Add</strong> to search {isBook ? 'books' : 'movies and TV shows'}</p>
            </>
          ) : (
      ...
      {showAdd && (
        <AddSheet defaultTab={forceType} onClose={() => setShowAdd(false)} onAdded={item => setItems(prev => [item, ...prev])} />
      )}
      {selected && (
