...
export function QuickAddModal({ onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-t-3xl md:rounded-2xl p-5 pb-8 md:pb-5 shadow-2xl space-y-4"
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            ...
            <input className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm bg-gray-50 dark:bg-gray-800 outline-none focus:border-indigo-400"
              value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} placeholder="e.g. Health & Body" />
          </div>
          <div>
        ...
        <button onClick={save} disabled={saving || !form.name.trim() || !form.category.trim()}
          className="w-full py-3 bg-indigo-600 text-white font-semibold rounded-2xl hover:bg-indigo-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
          <Check size={18} />
