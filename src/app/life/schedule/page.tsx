...
export default function SchedulePage() {
  ...
  async function saveDay(day: string) {
    await fetch(`/api/schedule/${day}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ _type:'day', label:dayLabel[day]??'', summary:daySummary[day]??'' }) })
  }

  async function handleSaveBlock(formData: BlockFormData) {
    if (modal?.mode === 'edit') {
      await fetch(`/api/schedule/${modal.block.id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify(formData) })
    } else {
      await fetch('/api/life/schedule', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ ...formData, day:activeDay }) })
    }
    setModal(null); await loadData()
  }

  async function handleDeleteBlock(block: ScheduleBlock) {
    await fetch(`/api/schedule/${block.id}`, { method:'DELETE' })
    setModal(null); await loadData()
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F9FAFB', padding:'24px 0' }}>
            ...
            <div style={{ background:'#F9FAFB',border:'1px solid #e5e7eb',borderRadius:12,padding:'12px 14px' }}>
              <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6 }}>
                <div>
                  ...
                  <p style={{ fontSize:11,color:'#9CA3AF',margin:0 }}>Shows only "Busy" — no event titles or details</p>
                </div>
      ...
      {modal && (
        <BlockModal
          initial={modal.mode === 'edit' ? { id:modal.block.id, startTime:modal.block.startTime, endTime:modal.block.endTime??'', name:modal.block.name, note:modal.block.note??'', category:modal.block.category, sacred:modal.block.sacred } : undefined}
