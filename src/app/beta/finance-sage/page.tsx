...
// One consistent glass recipe, used everywhere — nav, hero, lists, gauges.
// No more artificial "shell vs card" split; the whole thing reads as one
// material sitting on the same sage backdrop, same as the reference.
const glass: React.CSSProperties = {
  background: 'rgba(255,255,255,0.38)',
  backdropFilter: 'blur(22px) saturate(160%)', WebkitBackdropFilter: 'blur(22px) saturate(160%)',
  border: '1px solid rgba(255,255,255,0.55)',
  boxShadow: '0 8px 28px rgba(60,90,60,0.12), inset 0 1px 0 rgba(255,255,255,0.55)',
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <svg width="88" height="88" viewBox="0 0 88 88">
        <circle cx="44" cy="44" r={r} fill="none" stroke="rgba(30,50,30,0.1)" strokeWidth="9" />
...
export default function FinanceSageBeta() {
  ...
  if (loading || !dashboard) {
    return (
      <div style={{ minHeight: '100dvh', position: 'relative' }}>
        <GrainMesh tone="sage" />
        <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(30,50,30,0.4)' }}>Loading…</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100dvh', color: '#1e2e1e', position: 'relative' }}>
      <GrainMesh tone="sage" />
      <IconRail tone="light" />
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: '20px 20px 60px 90px' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {TABS.map(t => (
                <span key={t} style={{
                  ...
                  background: t === 'Overview' ? 'rgba(91,138,99,0.9)' : 'transparent',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {[['finance-dark', 'Dark'], ['finance-sage', 'Sage'], ['finance-photo', 'Photo']].map(([href, label]) => (
              <Link key={href} href={`/beta/${href}`} style={{
                ...
                background: href === 'finance-sage' ? 'rgba(30,50,30,0.85)' : 'rgba(30,50,30,0.1)',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16 }}>
          <div>
            <p style={{ fontSize: 15, color: 'rgba(30,50,30,0.6)' }}>Hey, Luka 👋</p>
          ...
          <div style={{ ...glass, width: 34, height: 34, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <RotateCcw size={15} color="rgba(30,50,30,0.5)" />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.6fr) 1fr', gap: 16 }}>
          ...
          <div>
            <div style={{ ...glass, borderRadius: 24, padding: 26 }}>
              <p style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'rgba(30,50,30,0.5)', marginBottom: 6 }}>Net worth</p>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
                ...
                {pace && (
                  <span style={{
                    ...
                    background: pace.status === 'ahead' ? 'rgba(200,80,60,0.16)' : pace.status === 'behind' ? 'rgba(91,138,99,0.18)' : 'rgba(30,50,30,0.1)',
              ...
              <p style={{ fontSize: 14, color: 'rgba(30,50,30,0.5)', marginTop: 4, fontFamily: 'monospace' }}>{formatRSD(dashboard.totals.totalRSD)}</p>
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 14, flexWrap: 'wrap' }}>
                {[['#5b8a63', 'Spent', formatRSD(totalSpend)], ['#8fb87a', 'Net income', formatRSD(dashboard.income.totalNetRSD)], ['#c8a35b', 'Deductions', formatRSD(dashboard.income.totalDeductions)]].map(([dot, label, val]) => (
                  <div key={label as string} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                    ...
                    <span style={{ color: 'rgba(30,50,30,0.6)' }}>{label}</span>
              ...
              <div style={{ marginTop: 22, height: 190, position: 'relative' }}>
                {peak && (
                  <div style={{
                    position: 'absolute', top: -6, right: 8, background: 'rgba(30,46,30,0.92)', color: '#fff', borderRadius: 12, padding: '8px 12px',
                    fontSize: 11, lineHeight: 1.5, boxShadow: '0 8px 20px rgba(30,50,30,0.25)', zIndex: 2, backdropFilter: 'blur(8px)',
                  }}>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
                    ...
                    <XAxis dataKey="name" tick={{ fontSize: 10, fill: 'rgba(30,50,30,0.5)' }} axisLine={false} tickLine={false} />
            ...
            <div style={{ display: 'flex', gap: 4, marginTop: 16, marginBottom: 12, overflowX: 'auto', paddingBottom: 2 }}>
              {SUBTABS.map(t => (
                <span key={t} style={{
                  ...
                  background: t === 'Goals' ? 'rgba(30,50,30,0.85)' : 'rgba(30,50,30,0.08)',
                  color: t === 'Goals' ? '#fff' : 'rgba(30,50,30,0.6)',
                }}>{t}</span>
              ))}
            </div>
            ...
            <div style={{ ...glass, borderRadius: 24, padding: 22 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {dashboard.accounts.map((a: any) => {
                  ...
                  return (
                    <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderTop: '1px solid rgba(30,50,30,0.08)' }}>
                      <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(91,138,99,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#4b7a53', flexShrink: 0 }}>
                        {a.name.slice(0, 1)}
                      </div>
                      ...
                      <span style={{ fontSize: 12, color: 'rgba(30,50,30,0.45)', width: 40, textAlign: 'right' }}>{share}%</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ ...glass, borderRadius: 24, padding: 22 }}>
              ...
              <p style={{ fontSize: 11, color: 'rgba(30,50,30,0.5)', marginBottom: 14 }}>vs. your usual pace</p>
            ...
            <div style={{ ...glass, borderRadius: 24, padding: 22 }}>
              ...
              <p style={{ fontSize: 12, color: 'rgba(30,50,30,0.5)', marginTop: 4 }}>after {formatRSD(dashboard.income.totalDeductions)} deductions</p>
            </div>
