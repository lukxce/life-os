...
export default function DocumentsPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {docs.length === 0 && (
        <div className="bg-surface/90 dark:bg-surface/70 rounded-2xl border border-black/5 dark:border-white/5 p-12 text-center">
        ...
        <section key={cat}>
          ...
          <div className="space-y-2">
            {catDocs.map(doc => {
              const status = statusFor(doc)
              return (
                <div key={doc.id} className="bg-surface/90 dark:bg-surface/70 rounded-2xl border border-black/5 dark:border-white/5 px-4 py-3.5 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{doc.name}</p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {status.icon}
