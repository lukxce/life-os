...
export default function InsightsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        {([['all', 'All Categories'], ['movers', 'Top Movers']] as const).map(([key, label]) => (
          <button key={key} onClick={() => setView(key)}
            className={cn('px-4 py-1.5 rounded-md text-sm font-medium transition-colors',
              view === key ? 'bg-surface/90 dark:bg-surface/70 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200')}>
            {label}
          </button>
        ))}
      </div>
      ...
      {/* Category breakdown */}
      {shown.length === 0 ? (
        <div className="bg-surface/90 dark:bg-surface/70 rounded-xl border border-black/10 dark:border-white/10 p-8 text-center text-gray-400">
        ...
        <div className="space-y-3">
          {shown.map(cat => {
            ...
            return (
              <div key={cat.category} className="bg-surface/90 dark:bg-surface/70 rounded-xl border border-black/10 dark:border-white/10 p-4">
