          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map(row => (
              <tr key={row.label} className="hover:bg-gray-50">
                ...
                <td className={`px-4 py-3 text-right font-semibold ${color === 'blue' ? 'text-blue-600' : color === 'red' ? 'text-red-600' : 'text-purple-600'}`}>{formatRSD(row.ytdRSD)}</td>
        ...
        {rows.map(row => (
          <div key={row.label} className="border border-gray-100 rounded-lg p-3">
            <div className="flex items-baseline justify-between mb-2">
              ...
              <span className={`font-semibold ${color === 'blue' ? 'text-blue-600' : color === 'red' ? 'text-red-600' : 'text-purple-600'}`}>{formatRSD(row.ytdRSD)}</span>
            </div>
