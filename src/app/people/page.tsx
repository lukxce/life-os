...
import { Plus, Pencil, Trash2, X, Phone, Search, Home } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-sm p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold">{contact ? 'Edit contact' : 'Add VIP contact'}</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="text" placeholder="e.g. 03-15" maxLength={5}
              className="w-full text-sm bg-gray-50 dark:bg-gray-800 rounded-xl px-3 py-2.5 outline-none border border-gray-200 dark:border-gray-700"
        ...
        <textarea placeholder="Note (optional)" rows={2}
          className="w-full text-sm bg-gray-50 dark:bg-gray-800 rounded-xl px-4 py-3 outline-none border border-gray-200 dark:border-gray-700 resize-none"
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => { if (name.trim()) { onSave({ name: name.trim(), emoji, color, birthday: birthday || null, reachOutFrequency: freq, note: note || null }); onClose() } }}
...
export default function PeoplePage() {
  ...
  async function markContacted(id: string) {
    ...
    await fetch(`/api/life/contacts/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lastContactDate: today }) })
    load()
  }
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 max-w-lg mx-auto space-y-4 pt-6">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Link href="/" style={{ fontSize: 11, color: '#9CA3AF', display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none' }}
              className="hover:text-blue-500 transition-colors">
              <Home size={11} /> Life OS
            </Link>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', margin: 0 }}>People</h1>
            <p className="text-sm text-gray-400 mt-0.5">VIP contacts · {overdue.length} need attention</p>
          </div>
          <button onClick={() => setModal({ mode: 'add' })} className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-semibold mt-5">
            <Plus size={16} /> Add
          </button>
        </div>
        ...
        {contacts.length === 0 && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-10 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">No VIP contacts yet</p>
            <p className="text-xs mt-1 text-gray-400 max-w-xs mx-auto">Add the people who matter most — family, close friends, key relationships. Get nudged when it's time to reach out.</p>
        ...
        {overdue.length > 0 && (
          <section>
            ...
            <div className="space-y-2">
              {overdue.map(c => (
                <ContactCard key={c.id} contact={c} onMarkContacted={() => markContacted(c.id)} onEdit={() => setModal({ mode: 'edit', contact: c })} onDelete={() => deleteContact(c.id)} />
              ))}
            </div>
          </section>
        )}
