  ...
  return (
    <div style={{ position:'absolute',top,left:0,right:0,height,background:color+'18',borderLeft:`3px solid ${color}`,borderRadius:6,padding:isShort?'2px 8px':'5px 10px',overflow:'hidden',boxSizing:'border-box' }}>
      <div style={{ display:'flex',alignItems:'center',gap:5,overflow:'hidden' }}>
        ...
        <span style={{ fontSize:11,fontWeight:700,color,overflow:'hidden',whiteSpace:'nowrap',textOverflow:'ellipsis' }}>
          {isBusy ? 'Busy' : ev.summary}
        </span>
        {!isBusy && ev.location && <span style={{ fontSize:10,flexShrink:0 }}>{virtual ? '💻' : '📍'}</span>}
      </div>
      {!isShort && !isBusy && ev.location && (
...
export default function SharePage() {
  ...
  return (
    <div style={{ minHeight:'100vh',background:'#F9FAFB',padding:'24px 16px 48px' }}>
      <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
        ...
        <div style={{ background:'#fff',border:'1px solid #e5e7eb',borderRadius:14,padding:'16px 16px 16px 0' }}>
          <div style={{ display:'flex' }}>
            ...
            <div style={{ flex:1,position:'relative',height:totalHeight }}>
              ...
              {dayEvents.map(ev => <EventBlock key={ev.uid + ev.start} ev={ev} isBusy={data.isBusy} />)}
            </div>
          </div>
        </div>
